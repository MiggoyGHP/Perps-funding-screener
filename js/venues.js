...
export const VENUES = {
  ...
  hyperliquid: {
    ...
    rateBasis: 'hourly, as charged (already 1/8 of the 8h-basis formula output)',
    annualizationNote: 'Rate is hourly → APR = rate × 8,760. The docs’ “8-hour” language is the internal formula only; API values are per-hour.',
    cap: '4% per hour',
  ...
  binance: {
    ...
    rateBasis: 'per that symbol’s own funding interval (8h default; per-symbol 4h/1h overrides)',
    annualizationNote: 'Rate is per interval → APR = rate × (24 ÷ interval h) × 365. Interval read per symbol from /fapi/v1/fundingInfo (HYPEUSDT = 4h).',
  ...
  bybit: {
    ...
    name: 'Bybit linear',
    defaultIntervalHours: 8,
    rateBasis: 'per that symbol’s own funding interval (per-symbol; instruments-info fundingInterval is in minutes)',
    annualizationNote: 'Rate is per interval → APR = rate × (8,760 ÷ interval h). Interval read per symbol from instruments-info (minutes ÷ 60).',
